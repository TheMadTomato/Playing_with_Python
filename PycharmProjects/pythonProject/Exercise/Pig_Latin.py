...
def pig_latinizer(sentence):
    sentence = sentence.split()
    for i in range(len(sentence)):
        sentence[i] = sentence[i][1:] + sentence[i][0] + "ay"
    return " ".join(sentence)


# take string input in english with no capitalization and punctuation
sentence = input("Enter a sentence in English: ").lower()

# output the sentence in pig latin
print(pig_latinizer(sentence))
